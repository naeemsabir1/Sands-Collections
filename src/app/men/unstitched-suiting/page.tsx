...
const materials = ['Italian Wool', 'Tropical Blend', 'Wool Blend', 'Pure Linen'];

export default function MensUnstitchedSuitingPage() {
    ...
    useEffect(() => {
        async function loadProducts() {
            ...
            try {
                ...
                const suitingProducts = allProducts.filter(p => p.category === 'mens-unstitched-suiting');
                setProducts(suitingProducts);
            } catch (error) {
                ...
                <div className="relative h-full container mx-auto px-4 lg:px-8 flex items-center">
                    ...
                    >
                        ...
                        <p className="text-white/80 text-lg md:text-xl font-light leading-relaxed">
                            Premium suiting fabrics for bespoke tailoring. Italian-style wool and tropical blends for the distinguished gentleman.
                        </p>
                    </motion.div>
                </div>
            </section>
                    ...
                    <>
                        <div className="mb-8 flex items-center justify-between">
                            <p className="text-medium-gray text-[15px]">
                                Showing <span className="font-bold text-charcoal">{filteredProducts.length}</span> premium fabrics
                            </p>
                        </div>
                        <ProductGrid products={filteredProducts} columns={4} />
                        {filteredProducts.length === 0 && (
                            <div className="text-center py-12 text-medium-gray">
                                No suiting fabrics found. Add products in the admin panel.
                            </div>
                        )}
                    </>
                )}
